...
export const Home: React.FC = () => {
  ...
  const yDrops = useTransform(dropsScroll, [0, 1], [100, -100]);
  ...
  return (
    <div className="w-full bg-transparent overflow-hidden">
        ...
        >
           <div className="z-20 flex flex-col items-center">
              <div className="mb-6 relative">
                 <h1 className="text-8xl md:text-[10rem] font-black tracking-tighter leading-[0.8] text-white mix-blend-difference drop-shadow-2xl text-center flex flex-col items-center">
                    <SplitText text="TUNNEL" delay={0.1} />
                    <span className="text-transparent bg-clip-text bg-gradient-to-b from-white to-gray-500 inline-block">
                      VISION
                    </span>
                 </h1>
              </div>
              ...
              <Reveal delay={0.6}>
                <p className="text-brand-gray text-lg md:text-xl font-light max-w-lg mx-auto leading-relaxed mb-10 border-l-2 border-brand-red/50 pl-6 text-left md:text-center md:border-l-0 md:pl-0 backdrop-blur-sm bg-black/20 p-4 rounded-r-lg">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
           ...
           <motion.div style={{ y: yDrops }} className="grid grid-cols-1 md:grid-cols-3 gap-8">
             {["7IbqNeOivzHYdXD7J6uHSq", "43uLpGO5OagtSVFSAoOIUl", "3tgbo7ajgLyIE8qbN3dNU8"].map((id, i) => (
                <Reveal key={id} delay={i * 0.1}>
                  <div className="group relative flex flex-col h-full bg-[#0F0F0F] rounded-xl overflow-hidden border border-white/5 hover:border-brand-red/30 transition-all duration-300 shadow-2xl hover:-translate-y-2">
                     <iframe 
             ...
             <Reveal width="100%">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                   {[
                      ...
                      <div key={i} className={`group relative p-8 rounded-2xl border transition-all duration-500 overflow-hidden ${tier.highlight ? 'bg-white/5 border-brand-red/50 shadow-[0_0_30px_rgba(225,6,0,0.1)]' : 'bg-[#0A0A0A] border-white/5'} flex flex-col items-center text-center hover:scale-105`}>
                         <div className="absolute inset-0 bg-gradient-to-br from-transparent via-transparent to-brand-red/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                         
                         <h5 className="text-white font-bold uppercase tracking-wider text-sm mb-2 relative z-10">{tier.name}</h5>
                         <div className={`text-3xl font-black mb-6 relative z-10 ${tier.highlight ? 'text-brand-red' : 'text-white'}`}>{tier.price}</div>
                         <ul className="text-xs text-brand-gray space-y-3 mb-8 relative z-10">
                            {tier.features.map((f, j) => <li key={j} className="flex gap-2 items-center justify-center"><span className="w-1 h-1 bg-brand-red rounded-full" />{f}</li>)}
                         </ul>
                         <Link to="/licensing" className="relative z-10">
                            <Button variant={tier.highlight ? 'primary' : 'outline'} className="w-full text-[10px] py-3">View Details</Button>
                         </Link>
                      </div>
                   ))}
                </div>
        ...
        <section className="py-24 border-t border-white/5">
           <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
              ...
              <div className="space-y-4">
                 <Reveal delay={0.2}>
                    <div className="group flex gap-6 p-6 border border-white/5 bg-white/[0.02] hover:bg-white/[0.05] transition-colors rounded-xl relative overflow-hidden">
                       <div className="absolute left-0 top-0 w-1 h-full bg-brand-red scale-y-0 group-hover:scale-y-100 transition-transform duration-300 origin-top" />
                 ...
                 <Reveal delay={0.3}>
                    <div className="group flex gap-6 p-6 border border-white/5 bg-white/[0.02] hover:bg-white/[0.05] transition-colors rounded-xl relative overflow-hidden">
                       <div className="absolute left-0 top-0 w-1 h-full bg-brand-red scale-y-0 group-hover:scale-y-100 transition-transform duration-300 origin-bottom" />
        ...
        {/* --- PRODUCTION CREDITS (TICKER) --- */}
        <section className="py-24 border-t border-white/5">
           <Reveal width="100%">
             <div className="text-center mb-16">
                <h4 className="text-xs font-bold uppercase tracking-[0.3em] text-brand-red mb-2">Selected Works</h4>
                <div className="text-3xl font-black text-white uppercase tracking-tight flex justify-center">
                    <SplitText text="PRODUCTION CREDITS" />
                </div>
             </div>
