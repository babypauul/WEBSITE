      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            {/* Branding / Logo */}
            <NavLink to="/" className="flex flex-col group cursor-hover z-50">
                <span className="text-2xl font-black tracking-tighter text-brand-red leading-none drop-shadow-[0_0_20px_rgba(225,6,0,0.4)] transition-all duration-500 group-hover:text-white group-hover:drop-shadow-[0_0_25px_rgba(255,255,255,0.6)]">
                  BABYPAUUL
                </span>
                <span className="text-[0.65rem] font-bold uppercase tracking-[0.35em] text-white/60 leading-none mt-1.5 pl-0.5 group-hover:text-brand-red transition-colors duration-300">
                  PRODUCER
                </span>
            </NavLink>
              ...
              {NAVIGATION.map((item) => (
                ...
                >
                  {({ isActive }) => (
                    <>
                      {item.label}
                      <span className={`absolute bottom-0 left-0 h-[1px] bg-brand-red shadow-[0_0_8px_rgba(225,6,0,0.8)] transition-all duration-300 ${isActive ? 'w-full' : 'w-0 hover:w-1/2'}`}></span>
                    </>
                  )}
                </NavLink>
              ))}
                  ...
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `block px-4 py-4 text-lg font-bold uppercase tracking-widest border-l-2 transition-all ${
                        isActive ? 'text-brand-red bg-white/5 border-brand-red' : 'text-brand-gray border-transparent hover:text-white hover:border-white/20'
                      }`
                    }
                  >
                    {item.label}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
           <div className="grid grid-cols-1 md:grid-cols-12 gap-12 mb-16">
              ...
              <div className="md:col-span-4">
                <Link to="/" className="flex flex-col mb-6 w-fit group">
              ...
              <div className="md:col-span-3 md:col-start-6">
                 ...
                 <ul className="space-y-4">
                    {NAVIGATION.map(item => (
                       <li key={item.path}>
                          <NavLink to={item.path} className="text-brand-gray hover:text-brand-red transition-colors text-sm hover:translate-x-2 inline-block duration-200">
                             {item.label}
                          </NavLink>
                       </li>
                    ))}
           ...
           <div className="border-t border-white/5 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
              ...
              <div className="flex gap-6 text-xs text-brand-gray/50 font-mono">
                 ...
                 <a href="#" className="hover:text-white transition-colors">Licensing Info</a>
              </div>
           </div>
        </div>
      </footer>
